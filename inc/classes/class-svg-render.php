...
class Svg_Render {
	...
	private function setup_hooks() {
		 ...
		 * Actions
		 */
		add_action( 'render_block', [ $this, 'render_icons_dynamically' ], 10, 2  );
	...
	private static function get_icons() {
		$ico   = include ELEMENTIFY_DIR_PATH . '/inc/helpers/icons-ico-array.php';
		$faico = include ELEMENTIFY_DIR_PATH . '/inc/helpers/icons-array.php';

		return apply_filters( 'elementify_svg_icons', array_merge( $ico, $faico ) );
	}
	...
	public static function render( $name, $fill = 'currentColor', $stroke_width = false, $title = '', $hidden = true, $extras = '', $echo = false ) {
		...
		if ( ! empty( self::$all_icons[ $name ] ) ) {
			...
			$svg .= '<svg class="ele-svg-icon" viewBox="' . $vb . '" ' . $preserve . ' fill="' . esc_attr( $fill ) . '"' . ( ! empty( $stroke_width ) ? ' stroke="currentColor" stroke-width="' . esc_attr( $stroke_width ) . '" stroke-linecap="round" stroke-linejoin="round"' : '' ) . ' xmlns="http://www.w3.org/2000/svg" ' . ( ! empty( $extras ) ? ' ' . $extras : '' ) . ( $hidden ? ' aria-hidden="true"' : ' role="img"' ) . '>';
			if ( ! empty( $title ) ) {
